...
@testset "Merging scores" begin
    @testset "Test 1" begin
        ...
        @testset "Loading excel file" begin
            csv_data =
                UniAdminTools.MergeScore.load_and_validate_data(input_fname, "", "", "", "")
            excel_data = UniAdminTools.MergeScore.load_and_validate_data(
                "example_scorings.xlsx",
                "example_scorings",
                "B1:H1",
                "A2:A12",
                "B2:H12",
            )
            for (csv_row, excel_row) in zip(eachrow.((csv_data, excel_data))...)
                for (csv_cell, excel_cell) in zip(csv_row, excel_row)
                    @test (ismissing(csv_cell) && ismissing(excel_cell)) ||
                          csv_cell == excel_cell
                end
            end
        end
